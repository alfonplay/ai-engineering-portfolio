...
def predict(token: str, uploaded_file: Image) -> requests.Response:
    ...
    response = requests.post(API_BASE_URL + "/model/predict", files=files, headers=headers)
    return response
...
def send_feedback(
    ...
    response = requests.post(API_BASE_URL + "/feedback", json=payload, headers=headers)
    return response
