...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    # Sum freight_value and product_weight_g per order
    aggregations = (
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    # Convert the purchase timestamp to proper datetime
    orders["order_purchase_timestamp"] = pd.to_datetime(orders["order_purchase_timestamp"])

    # Keep only 2017 orders
    ...
    order_purchase_amount_per_date = filtered_dates.groupby(
        filtered_dates["order_purchase_timestamp"].dt.normalize()
    )["order_id"].count()

    # Build the result with order count, date, and whether each day was a holiday
    holiday_dates = pd.to_datetime(holidays["date"])
    result_df = pd.DataFrame({
        "order_count": order_purchase_amount_per_date.values,
        "date": pd.to_datetime(order_purchase_amount_per_date.index),
        "holiday": pd.to_datetime(order_purchase_amount_per_date.index).isin(holiday_dates),
    })
