...
def train_test_split_and_feature_extraction(df, test_size=0.3, random_state=42):
    ...
    text_columns = sorted(
        [col for col in df.columns if col.startswith('text_') and col[5:].isdigit()],
        key=lambda x: int(x.split('_')[1])
    )
    image_columns = sorted(
        [col for col in df.columns if col.startswith('image_') and col[6:].isdigit()],
        key=lambda x: int(x.split('_')[1])
    )
    label_columns = [col for col in df.columns if col == 'class_id']
